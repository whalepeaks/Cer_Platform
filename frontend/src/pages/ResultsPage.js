...
function ResultsPage() {
  ...
  const handleFeedbackAll = useCallback(async () => {
    const unFeedbackedQuestions = resultData.answeredQuestions.filter(q => !q.ai_comment);
    if (unFeedbackedQuestions.length === 0) {
      alert('모든 문제의 해설이 이미 존재합니다.');
      return;
    }
    setLoadingStates(prev => ({ ...prev, feedbackAll: true }));
    const isConfirmed = window.confirm(`총 ${unFeedbackedQuestions.length}개의 문제에 대한 AI 해설을 생성합니다. API 사용량이 많이 발생할 수 있습니다. 계속하시겠습니까?`);
    if (!isConfirmed) {
      setLoadingStates(prev => ({ ...prev, feedbackAll: false }));
      return;
    }
    for (const question of unFeedbackedQuestions) {
      ...
      <Card className="mb-4 text-center">
        ...
        <Card.Body>
          <Row>
            ...
            <Col md={4} className="d-grid mb-2 mb-md-0">
              <Button variant="warning" onClick={handleFeedbackAll} disabled={loadingStates.feedbackAll}>
            ...
            <Col md={4} className="d-grid">
              {finalScore !== null ? (
                ...
                <Button variant="success" onClick={handleGetFinalScore} disabled={loadingStates.finalScore}>
            ...
            <ListGroup.Item as="li" key={item.questionId} className="mb-3 p-3 shadow-sm">
              ...
              <div className="mt-3">
                <Button variant="outline-info" size="sm" onClick={() => handleAction('feedback', item.questionId)} disabled={loadingStates[`feedback-${item.questionId}`]}>
                  {loadingStates[`feedback-${item.questionId}`] ? '생성 중...' : item.ai_comment ? (isFeedbackVisible ? '해설 숨기기' : 'AI 맞춤 해설 보기') : 'AI 맞춤 해설 생성'}
                </Button>
